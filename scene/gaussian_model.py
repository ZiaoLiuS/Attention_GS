...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float):
        ...
        if test_flag == False:
            ...
            for i in range(num_sclaes):
                length = num_pts * 8**i
                dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points[start_idx:start_idx+length])).float().cuda()), 0.0000001)
                s = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
                scales[start_idx:start_idx + length] = s
                start_idx += length
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, abe_split=False):
        ...
        if abe_split:
            ...
            new_tmp_radii = self.tmp_radii[selected_pts_mask].repeat(BACK_N)
        ...
        new_scaling = self.scaling_inverse_activation(self.get_scaling[selected_pts_mask].repeat(N,1) / (0.7*N))
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii, abe_split=False):
        grads = self.xyz_gradient_accum / self.denom
        grads[grads.isnan()] = 0.0

        self.tmp_radii = radii
        self.densify_and_clone(grads, max_grad, extent)
        self.densify_and_split(grads, max_grad, extent,abe_split=abe_split)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, pixels,flag):
